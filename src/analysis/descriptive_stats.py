...
def is_technical_column(column: str) -> bool:
    """Detect pipeline-generated columns that should not be primary analytical variables."""
    return (
        column.endswith("_clean")
        or column.endswith("_invalid_flag")
...
def detect_id_columns(df: pd.DataFrame) -> list:
    ...
    for column in df.columns:
        ...
        if column_lower in known_id_columns or column_lower.endswith("_id"):
...
def detect_round_number_max(series: pd.Series) -> dict | None:
    ...
    is_round_number = max_value % 100 == 0 or max_value % 1000 == 0
...
def compute_descriptive_stats(df: pd.DataFrame) -> dict:
    ...
    analytical_df = df.drop(columns=technical_columns, errors="ignore")

    numeric_df = analytical_df.select_dtypes(include="number").drop(
        columns=id_columns,
        errors="ignore",
    )
    ...
    for column in continuous_columns:
        ...
        if median_value != 0:
            mean_median_gap_pct = (
                abs(mean_value - median_value) / abs(median_value) * 100
            )
        else:
    ...
    for column in low_cardinality_numeric_columns:
        ...
        percents = (
            series.value_counts(normalize=True).sort_index() * 100
        ).round(2)
    ...
    technical_df = df[technical_columns] if technical_columns else pd.DataFrame()

    technical_summary = (
        summarize_categorical_columns(technical_df, [])
